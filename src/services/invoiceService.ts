...
import type { InvoiceRow, InvoiceItem, InvoiceFilters, PaginatedInvoices } from '@/types/invoice'
...
export async function fetchVendors(clientId: string): Promise<string[]> {
  ...
  if (error || !data) return []
  return [...new Set(data.map((r: any) => r.vendor).filter(Boolean))] as string[]
}
...
export async function fetchCurrencies(clientId: string): Promise<string[]> {
  ...
  if (error || !data) return []
  return [...new Set(data.map((r: any) => r.currency).filter(Boolean))] as string[]
}